...
const ResetPassword = () => {
    ...
    async function handle_submit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await patch_auth_request('auth/reset-user-password', {new_password: auth.password}) as AxiosResponseHeaders
        ...
        <section className="w-full h-full px-[1rem] md:px-[2rem] lg:px-[4rem] flex flex-col items-center justify-center gap-8 relative overflow-y-hidden">
            <div className="w-full h-full flex flex-col items-center justify-center gap-8 my-10">
                ...
                <form onSubmit={handle_submit} className="w-full flex flex-col gap-5 justify-center items-start ">
                    ...
                    <Button size="sm" className="mt-5 w-full h-[55px] bg-[#306CE9] text-white hover:bg-[#306CE9]/90 transition-all duration-300 font-mont font-semibold rounded text-[15.5px]"  disabled={auth.password === '' || auth.confirm_password === '' }>
