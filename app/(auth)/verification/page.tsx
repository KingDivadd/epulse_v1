    ...
    async function handle_submit(e:any) {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                const {gender, phone_number, country_code, } = response.data.user


                if ((user_information?.role === 'patient' || user_information?.patient_id) && (!gender || !phone_number || !country_code)) {
