...
const PhoneInputComponent = ({country_code, phone_number, on_change}:PhoneInputProps) => {
    ...
    const { setCountry_dial_code, country_dial_code} = useChat()
    ...
    return (
        <div className="flex items-center gap-2 w-full">
            <select
                ...
                className="border border-slate-400 rounded px-1 h-[50px] w-1/3 text-[13px] font-mont" >
                <option value="" className='text-[12px]' >Select</option>
                {
                    countries.map((country,ind)=>{

                        return(
                            <option key={ind} value={country.dial_code} className='flex gap-5 text-[12px] font-mont '>
                                {country.name}
                            </option>
                        )
                    })
