...
const Sidebar = () => {
    ...
    return (
        <div className="w-full  flex flex-col gap-3 h-full bg-white ">
            <span className="w-full flex justify-center items-center h-24  "> <SidebarHeading /> </span>

            <article className="w-full flex flex-col pr-2 lg:pr-5 relative">
                {
                    route_list.map((route, index) => (
                        <Link key={index} href={route.path} className={current_route === route.path ? "pl-5 lg:px-8 current-nav-item bg-blue-100  ": "pl-5 lg:px-8 nav-item "} onClick={()=>  setCurrent_route(route.path)}>
